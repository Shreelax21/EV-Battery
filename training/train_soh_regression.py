...
def main():
    # Load tabular data (features and labels)
    ...
    # Format metrics in a clean table
    table_data = []
    for split, metrics_dict in allm.items():
        table_data.append([
            split.capitalize(),
            round(metrics_dict["MAE"], 6),
            round(float(metrics_dict["RMSE"]), 6),
            round(metrics_dict["R2"], 6)
        ])

    headers = ["Dataset", "MAE", "RMSE", "R² Score"]
